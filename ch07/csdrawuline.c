...
static void Redraw(w)
    Widget w;
{
    XmStringDrawUnderline(XtDisplay(w), XtWindow(w), fontlist, string, gc, x, y,
            width, XmALIGNMENT_CENTER, XmSTRING_DIRECTION_L_TO_R, NULL, uline);
}
    ...
    char **argv;
{
    ...
    Arg      al[20];
    int      ac;
    static char *text = "Draw Underline";
    static char *textul = "Underline";

    XtSetLanguageProc(NULL, NULL, NULL);
    ac = 0;
    toplevel = XtAppInitialize(&app_context, "Csdrawuline", NULL, 0,
    ...
    /* (7) Create compound strings */
    string = XmStringCreateLocalized(text);  /* whole text */
    uline = XmStringCreateLocalized(textul); /* under lined text */
